...
const pendingConnectionTests = new Set();
const pendingConnectionTestTimers = new Map();

const MAX_HISTORY_ITEMS = 30;
const CONNECTIONS_STORAGE_KEY = 'connectionsState';
const CONNECTION_TEST_TIMEOUT_MS = 18000;

function setConnectionTestPending(connectionId, nextPending) {
  const id = String(connectionId || '').trim();
  if (!id) return;

  const existingTimer = pendingConnectionTestTimers.get(id);
  if (existingTimer) {
    clearTimeout(existingTimer);
    pendingConnectionTestTimers.delete(id);
  }

  if (!nextPending) {
    pendingConnectionTests.delete(id);
    return;
  }

  pendingConnectionTests.add(id);
  const timeoutId = setTimeout(async () => {
    pendingConnectionTestTimers.delete(id);
    if (!pendingConnectionTests.has(id)) return;
    pendingConnectionTests.delete(id);
    ...
    conn.status = 'ready';
    conn.lastError = 'Connection test timed out. Check API URL/key and network.';
    await saveConnectionsState();
    renderConnections();
  }, CONNECTION_TEST_TIMEOUT_MS);
  pendingConnectionTestTimers.set(id, timeoutId);
}
...
function renderShortcutsPanel() {
  ...
  shortcutsDropdown.innerHTML = `
    <div class="shortcut-item">
      <span class="shortcut-text">Saved shortcuts will appear here.</span>
    </div>
  `;
}
...
function setShortcutsPanelOpen(nextOpen) {
  ...
  if (shortcutsPanelOpen) {
    if (!shortcutsDropdown.innerHTML.trim()) renderShortcutsPanel();
...
function renderSuggestionCloud() {
  ...
  const prompts = [
    'Find me the cheapest flight to Barcelona next week',
    'Go to Amazon and find headphones under $50',
    'Read this page and summarize it in 3 sentences',
    'Fill out this form with my info',
    'Compare prices for this product across sites',
  ];
...
function showResult(result) {
  ...
  if (result.success) {
    ...
    let html = `<details class="result-collapse" open><summary><div class="result-header">${icon('check-circle')} Done (${result.steps} steps): ${escapeHtml(result.summary)}</div></summary>`;
    ...
    resultBanner.dataset.mode = '';
    resultBanner.innerHTML = `<details class="result-collapse" open><summary><div class="result-header">${icon('x-circle')} Failed (${result.steps} steps): ${escapeHtml(result.reason || 'Unknown error')}</div></summary></details>`;
  }
...
btnSaveShortcut?.addEventListener('click', (event) => {
...
function renderConnectionCard(item) {
  ...
  const isTesting = pendingConnectionTests.has(item.id);
    ...
    <div class="conn-card${connected ? ' connected' : ''}" data-conn-id="${escapeAttr(item.id)}">
      ...
      <div class="conn-config${isOpen ? ' show' : ''}">
        ...
        <div class="conn-config-actions">
          ...
          <button class="conn-btn" data-action="test"${isTesting ? ' disabled' : ''}>${isTesting ? 'Testing...' : 'Test'}</button>
...
function renderConnections() {
  ...
  connRoutingList.innerHTML = routes.map((r) => {
    ...
    <div class="conn-route-row" data-route-id="${escapeAttr(r.id)}">
      <button class="conn-toggle${on ? ' on' : ''}" title="Toggle ${escapeAttr(r.label)}"></button>
  ...
  const wireCardList = (root) => {
    root.querySelectorAll('.conn-card').forEach((card) => {
      ...
      card.querySelector('[data-action="test"]')?.addEventListener('click', async () => {
        ...
        conn.lastError = '';
        conn.status = 'ready';
        setConnectionTestPending(connId, true);
        await saveConnectionsState();
        renderConnections();
  ...
  connRoutingList.querySelectorAll('.conn-route-row .conn-toggle').forEach((toggleEl) => {
    toggleEl.addEventListener('click', async () => {
      const row = toggleEl.closest('.conn-route-row');
      const routeId = row?.dataset.routeId;
      if (!routeId) return;
      connectionsState.routing = connectionsState.routing || {};
      connectionsState.routing[routeId] = !connectionsState.routing[routeId];
      await saveConnectionsState();
      renderConnections();
    });
  });
...
async function applyConnectionTestResult(msg) {
  ...
  setConnectionTestPending(msg.connectionId, false);
  if (!connectionsState) return;
