...
8. When you need multiple independent observations from the same page (e.g. find + extract_structured, or read_page + find_text), call them in a SINGLE response as parallel tool calls. They will execute concurrently for faster results. Only read-only tools (read_page, get_page_text, extract_structured, find, find_text, list_tabs) are eligible — do not batch mutating actions together.
9. On search-results pages, do not keep re-reading the SERP. After 1-2 observations, open an actual result page.
10. If a query gives no signal, reformulate it or change source; never spam the same query.
