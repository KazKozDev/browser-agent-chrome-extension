...
export const contextMethods = {
  ...
  _buildTaskStateSystemMessage() {
    ...
    let approvedPlan = 'None.';
    if (typeof this._approvedPlanText === 'string' && this._approvedPlanText.trim()) {
      approvedPlan = this._approvedPlanText.trim().slice(0, 2000);
    }
  ...
  _buildMessagesForLLM(messages) {
    ...
    if (messages.length === 0) return [stateMessage];
    // Optimization to preserve KV cache: put the changing stateMessage at the end, not the beginning
    if (messages.length >= 2) {
      return [...messages.slice(0, messages.length - 1), stateMessage, messages[messages.length - 1]];
    }
  ...
  _retrieveRelevantHistory(limit = HISTORY_RAG_QUERY_TOP_K, maxChars = 1200) {
    ...
    const scored = entries
      .map((entry) => {
        ...
        if (!text) return null;
        // ── Opt 8: Cache embeddings on the entry object to avoid recomputation ──
        if (!entry._embedding) {
          entry._embedding = buildHashedEmbedding(text, HISTORY_RAG_EMBEDDING_DIM);
        }
        const emb = entry._embedding;
  ...
  _compressHistory(messages) {
    ...
    for (let i = messages.length - 1; i >= 0; i--) {
      ...
      if (msg.role === 'assistant') {
        if (typeof msg.content === 'string' && msg.content.startsWith('[REFLECTION]')) {
          // ── Opt 3: Compress reflection messages more aggressively ──
          // Keep the 2 most recent reflections in full; compress older ones.
          // Reflections are already captured in the state tracker, so older ones
          // are redundant context that wastes tokens.
          if (assistantTurns >= 2) {
      ...
      if (assistantTurns >= 2) {
        if (msg.role === 'user' && Array.isArray(msg.content)) {
          ...
          if (hasImage) {
            // Extract any text analysis from this vision turn before compressing it
            const textPart = msg.content.find(c => c.type === 'text');
            const analysisText = String(textPart?.text || '').trim().slice(0, 300);
            const summaryText = analysisText
              ? `Snapshot summary: ${analysisText}`
              : 'Snapshot summary: [screenshot analysis from this turn omitted]';
            msg.content = summaryText;
            // Index the summary for RAG retrieval so it remains searchable
            this._indexHistoryChunkForRetrieval(summaryText, { source: 'vision_compress' });
          }
        }

        // ── Opt 3: Compress large read-only tool results in older turns ──
        // get_page_text and read_page results from older turns are rarely needed
        // verbatim — the key facts are captured in the state tracker.
        if (msg.role === 'tool' && assistantTurns >= 4) {
          const content = String(msg.content || '');
          if (content.length > 800) {
            // Check if this is a page-content result (wrapped in <page_content> tags)
            if (content.includes('<page_content>')) {
              msg.content = content.slice(0, 400) + '\n...[page content truncated — see state tracker for key facts]';
            }
          }
        }
      }
  ...
  /**
   * Tools whose results contain untrusted page content — wrapped in <page_content> tags
   * so the model can distinguish trusted instructions from untrusted web data.
   */
  _PAGE_CONTENT_TOOLS: new Set(['read_page', 'get_page_text', 'find', 'find_text', 'extract_structured']),

  _serializeToolResultForLLM(toolName, result) {
    let safe = result;

    // Never feed base64 blobs into conversation history.
    if (toolName === 'computer' && safe?.imageBase64) {
      safe = {
        ...safe,
    ...
    // Compact or full read_page serialization.
    if (toolName === 'read_page' && safe?.tree) {
      if (safe?._readMode !== 'full') {
        // Compact mode: flat list of interactive elements only — ~7× smaller context footprint.
        safe = this._buildInteractiveRefsFromReadPage(safe);
      } else {
        // Full mode: compress but keep tree structure.
    ...
    // Wrap page-sourced content in content-boundary tags so the model treats
    // it as untrusted data rather than trusted instructions (prompt injection defense).
    if (this._PAGE_CONTENT_TOOLS.has(toolName)) {
      return `<page_content>\n${output}\n</page_content>`;
    }
    return output;
  },
  ...
  /**
   * Extract only interactive elements (nodes with agent IDs) from a read_page result
   * and return them as a compact flat list instead of the full tree.
   * Produces ~7× less context than _compressReadPageForLLM while retaining all actionable refs.
   */
  _buildInteractiveRefsFromReadPage(result) {
    const refs = [];
    const seen = new Set();

    const walk = (node) => {
      if (!node || typeof node !== 'object') return;
      // Only include nodes that have an agent ID (interactive/focusable elements)
      if (node.id !== undefined && node.id !== null && !seen.has(node.id)) {
        seen.add(node.id);
        const ref = { id: node.id };
        if (node.role) ref.role = node.role;
        if (node.name) ref.name = String(node.name).slice(0, 80);
        if (node.tag) ref.tag = node.tag;
        if (node.state && typeof node.state === 'object' && Object.keys(node.state).length > 0) {
          ref.state = node.state;
        }
        refs.push(ref);
      }
    ...
    return {
      ...
      url: result?.url || '',
      title: result?.title || '',
      interactiveCount: result?.interactiveCount ?? refs.length,
      mode: 'compact',
      refs,
    };
  },
