export const stateMethods = {
  getCheckpointState(maxHistoryItems = 200) {
    ...
    const sanitizeResult = (tool, result) => {
      ...
      if (tool === 'computer' && safeResult?.imageBase64) {
        safeResult = {
          ...safeResult,
    ...
    return {
      ...
      stateSnapshots,
      reflectionState,
      pendingVerification: this._pendingVerification && typeof this._pendingVerification === 'object'
      ...
      activePauseKind: String(this._activePauseKind || ''),
      pendingPlanText: String(this._pendingPlanText || ''),
      approvedPlanText: String(this._approvedPlanText || ''),
      pendingIntervention: this._pendingIntervention && typeof this._pendingIntervention === 'object'
        ? this._pendingIntervention
        : null,
  ...
  _normalizeToolArgs(name, args) {
    ...
    const isComputerInteraction = name === 'computer' && ['click', 'type', 'select', 'hover'].includes(String(normalized.action || ''));
    if ((isComputerInteraction || name === 'form_input') && typeof normalized.target === 'string') {
      const trimmed = normalized.target.trim();
      if (/^\d+$/.test(trimmed)) {
        normalized.target = Number(trimmed);
      }
    }
    if (name === 'tabs_context' && (normalized.action === 'switch' || normalized.action === 'close')) {
    ...
    if (name === 'tabs_context' && normalized.action === 'switch_frame') {
    ...
    if (name === 'navigate' && normalized.action === 'reload') {
      if (normalized.bypassCache !== undefined) {
        normalized.bypassCache = this._normalizeBoolean(normalized.bypassCache);
      }
    }
    const isClick = name === 'computer' && normalized.action === 'click';
    if (isClick && normalized.confirm === undefined) {
    ...
    const isType = name === 'computer' && normalized.action === 'type';
    if (isType && normalized.enter !== undefined) {
      normalized.enter = this._normalizeBoolean(normalized.enter);
    }
    if (isType && normalized.text !== undefined && normalized.text !== null) {
    ...
    if (name === 'tabs_create' && typeof normalized.url === 'string') {
      normalized.url = normalized.url.trim();
      if (normalized.active === undefined) normalized.active = true;
    }
    if (name === 'computer' && normalized.action === 'scroll') {
    ...
    if (name === 'computer' && normalized.action === 'wait_for') {
