export const coverageMethods = {
  _isVerificationObservationTool(tool = '', args = {}, result = null) {
    if (result?.success === false) return false;
    const name = String(tool || '').trim();
    const action = String(args?.action || '').trim();
    if (['read_page', 'get_page_text', 'find', 'find_text', 'extract_structured', 'read_console', 'read_network'].includes(name)) {
      return true;
    }
    if (name === 'navigate' && String(result?.pageText || '').trim().length >= 40) {
      return true;
    }
    if (name === 'computer' && action === 'wait_for') {
      return true;
    }
    return false;
  },

  _actionRequiresVerification(tool = '', args = {}, result = null) {
    if (result?.success === false) return false;
    const name = String(tool || '').trim();
    const action = String(args?.action || '').trim();
    if (name === 'computer') {
      return ['click', 'select', 'press_key'].includes(action) || (action === 'type' && !!args?.enter);
    }
    return ['form_input', 'javascript'].includes(name);
  },

  _defaultExpectedOutcome(tool = '', args = {}) {
    const name = String(tool || '').trim();
    const action = String(args?.action || '').trim();
    if (name === 'computer') {
      if (action === 'click') return 'the clicked control should change the page state, status, or visible UI';
      if (action === 'select') return 'the selected option should change the page state or visible selection';
      if (action === 'press_key') return `pressing ${String(args?.key || 'the key')} should change the page state or focused UI`;
      if (action === 'type' && !!args?.enter) return 'submitting the typed input should change the page state or visible UI';
    }
    if (name === 'form_input') return 'the form control should show the updated value or checked state';
    if (name === 'javascript') return 'the scripted DOM change should be visible in page state';
    return 'the page should show the intended state change';
  },

  _buildPendingVerificationRecord(step, tool = '', args = {}, actionMeta = {}) {
    const name = String(tool || '').trim();
    const action = String(args?.action || '').trim();
    const actionLabel = name === 'computer' && action ? action : name;
    const expectedOutcome = String(actionMeta?.expected_outcome || '').trim().slice(0, 240)
      || this._defaultExpectedOutcome(name, args);
    return {
  ...
  _updatePendingVerificationAfterAction(step, tool = '', args = {}, actionMeta = {}, result = null) {
    if (result?.success === false) return;
    if (this._isVerificationObservationTool(tool, args, result)) {
      const pending = this._pendingVerification && typeof this._pendingVerification === 'object'
        ? this._pendingVerification
        : null;
      if (pending && Number(step) > Number(pending.step)) {
        this._pendingVerification = null;
      }
      return;
    }
    if (this._actionRequiresVerification(tool, args, result)) {
      this._pendingVerification = this._buildPendingVerificationRecord(step, tool, args, actionMeta);
    }
  },

  _validateDoneVerification() {
    const pending = this._pendingVerification && typeof this._pendingVerification === 'object'
      ? this._pendingVerification
      : null;
    ...
    return {
      ok: false,
      reason: pending.expectedOutcome
        ? `Completion rejected: the last mutating action is not verified yet. Verify that ${pending.expectedOutcome}.`
        : 'Completion rejected: the last mutating action is not verified yet.',
      hint: {
        strategy: 'verify_postcondition',
        nextTool: 'read_page',
        args: {},
        avoidRepeat: false,
      },
  ...
  _validateDoneQuality(summary = '', answer = '') {
    ...
    const infoGoal = /(find|search|check|look\s*up|lookup|news|price|weather|who|what|when|where|how\s*many|which|rate)/i.test(goalText);
    const processOnly = /(clicked|typed|navigated|opened|pressed|scrolled|filled)/i.test(combined);
    const hasFactSignal = (
  ...
  _extractCoverageKeywords(text) {
    const stopwords = new Set([
      ...
      'and', 'then', 'after', 'that', 'this', 'how', 'what', 'to', 'for', 'or', 'need', 'do', 'make',
      'find', 'check', 'open', 'go', 'in', 'on', 'by', 'from', 'but',
    ]);
