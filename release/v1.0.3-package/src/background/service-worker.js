  ...
  switch (msg.type) {
    case 'startTask': {
      ...
      activeAgent.onStep = (step) => {
        // 'thinking' events are transient UX indicators — do not buffer them for replay
        // (they would show stale "Reasoning…" indicators when sidepanel reconnects).
        const bufferIfDisconnected = step?.type !== 'thinking';
        sendToSidePanel({ type: 'step', step }, { bufferIfDisconnected });
        if (bufferIfDisconnected) {
