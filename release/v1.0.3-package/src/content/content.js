...
(() => {
    ...
    for (const root of roots) {
      ...
      const walker = activeDoc.createTreeWalker(root, NodeFilter.SHOW_TEXT, {
        acceptNode(node) {
          ...
          // Filter boilerplate by walking up the DOM — use only standardized signals
          // (semantic HTML tags and ARIA roles) which are reliable and unambiguous.
          // Class/ID name pattern matching is intentionally omitted: substring matches
          // like /nav/ falsely reject content inside "navcontainer", "navbar-wrap", etc.
          let curr = parent;
          while (curr && curr !== activeDoc.body && curr !== activeDoc.documentElement) {
            if (blockedTags.has(curr.tagName)) return NodeFilter.FILTER_REJECT;
            const role = String(curr.getAttribute('role') || '').toLowerCase();
            if (['banner', 'navigation', 'contentinfo', 'complementary', 'dialog', 'alertdialog'].includes(role)) {
  ...
  /**
   * Raw page text extraction with minimal filtering — only strips script/style tags.
   * Used as fallback when structured extraction returns nothing (e.g. non-semantic page layouts).
   */
  function collectRawPageText(activeDoc, maxChars = 8000) {
    const SCRIPT_ONLY = new Set(['SCRIPT', 'STYLE', 'NOSCRIPT']);
    const body = activeDoc.body || activeDoc.documentElement;
    if (!body) return { text: '', truncated: false };
    const walker = activeDoc.createTreeWalker(body, NodeFilter.SHOW_TEXT, {
      acceptNode(node) {
        const parent = node.parentElement;
        if (!parent) return NodeFilter.FILTER_REJECT;
        if (SCRIPT_ONLY.has(parent.tagName)) return NodeFilter.FILTER_REJECT;
  ...
  function getPageText(payload = {}) {
    ...
    // Fallback: structured extraction returned nothing (common on pages with non-semantic layout
    // or when the content root is accidentally inside a blocked ancestor tag).
    // Re-extract with minimal filtering (script/style only) to avoid losing real content.
    if (!text && !viewportOnly && scope !== 'selector') {
      const raw = collectRawPageText(activeDoc, maxChars);
      if (raw.text) {
        return {
          url: activeView.location.href,
          title: activeDoc.title,
          ...
          rawFallback: true,
          rawFallbackNote: 'Structured extraction returned empty; raw text extracted with minimal filtering.',
        };
      }
    }
  ...
  function extractStructured(payload = {}) {
    ...
    const wantsProducts = /product|products|item|items|headphone|headphones|earphones/i.test(`${hint} ${selector}`);
