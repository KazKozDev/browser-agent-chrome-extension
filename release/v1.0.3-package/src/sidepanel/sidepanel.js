...
sendBtn.addEventListener('click', () => {
  if (isPaused) {
    if (String(activeInterventionDetails?.type || '') === 'planApproval') {
...
function renderStep(step) {
  ...
  if (step.type === 'thinking') {
    // Transient "thinking" indicator shown while reflection LLM call is in progress.
    // Replaced by the actual thought/action when the call completes.
    el.className = 'step thinking';
    el.setAttribute('data-thinking-step', String(step.step));
    const header = document.createElement('div');
    header.className = 'step-header';
    header.innerHTML = `${icon('thought')} Step ${step.step} <span class="thinking-dots">…</span>`;
    ...
    // Remove any pending "thinking" indicator for this step
    removeThinkingIndicator(step.step);
    ...
    // Remove all pending "thinking" indicators on error (robust approach)
...
// Remove thinking indicator for a specific step
function removeThinkingIndicator(step) {
  if (step === undefined || step === null) return;
  const thinkingEl = stepsContainer.querySelector(`[data-thinking-step="${step}"]`);
  if (thinkingEl) thinkingEl.remove();
}

// Remove all thinking indicators (used on errors or task completion)
function removeAllThinkingIndicators() {
  const thinkingEls = stepsContainer.querySelectorAll('.step.thinking');
...
function showManualIntervention(details = {}) {
  ...
  const isPlanApproval = String(details.type || '') === 'planApproval' || String(details.kind || '') === 'plan_approval';
  ...
  if (isPlanApproval) {
    const plan = String(details.plan || '').trim();
    const goal = String(details.goal || '').trim();
    resultBanner.innerHTML = `
      <div class="result-header">${icon('bolt')} Plan approval required</div>
      <div style="margin-top:6px;color:var(--text);font-weight:400;">${escapeHtml(message)}</div>
      ${goal ? `<div style="margin-top:6px;color:var(--text2);font-size:11px;">Goal: ${escapeHtml(goal)}</div>` : ''}
      ${plan ? `<pre style="margin-top:8px;white-space:pre-wrap;">${escapeHtml(plan)}</pre>` : ''}
      <div style="margin-top:8px;color:var(--text2);font-size:11px;">Approve to let the agent start executing this plan.</div>
      <div class="manual-actions">
        <button id="manualApprovePlanBtn">Approve</button>
        <button id="manualStopBtn">Stop</button>
      </div>
    `;
  } else if (isJsDomainPermission) {
  ...
  document.getElementById('manualApprovePlanBtn')?.addEventListener('click', () => {
    sendMsg({ type: 'approvePlan' });
  });
